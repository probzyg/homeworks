...
public class DatePeriod {
    ...
    public DatePeriod intersection(DatePeriod otherPeriod) {
        ...
        if (overlapMessage != null) {
            ...
            DatePeriod overlap = new DatePeriod(intersectionStart, intersectionEnd);
            System.out.println(overlapMessage + " Overlap: " + overlap.getStart() + "-" + overlap.getEnd());
            return null;
    ...
    private String doesPeriodCompletelyInclude(DatePeriod otherPeriod) {
        if (start.isBefore(otherPeriod.getStart()) && end.isAfter(otherPeriod.getEnd())) {
            return "First period completely includes the second!";
        } else if (otherPeriod.getStart().isBefore(start) && otherPeriod.getEnd().isAfter(end)) {
            return "Second period completely includes the first!";
        } else {
            return null;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        LocalDate secondEnd = LocalDate.of(2022, 1, 13);
